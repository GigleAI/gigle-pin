...
SCAN=$(scripts/i18n-scan.sh 2>&1)
RC=$?
if [[ $RC -ne 0 ]]; then
  printf '%s\n' "$SCAN" >&2
  exit "$RC"
fi
printf '%s\n' "$SCAN" | grep -E "placeholder|untranslated +[1-9]|❌" || true
...
xcodebuild -project Pin.xcodeproj -scheme Pin -configuration "$CFG" -derivedDataPath build build 2>&1 \
  | tee build/build.log | grep -E "error:|warning: unre|BUILD (SUCCEEDED|FAILED)" | sort -u
RC=${PIPESTATUS[0]}
if [[ $RC -ne 0 ]]; then
  echo "Build failed (log: build/build.log)" >&2
  exit "$RC"
fi
